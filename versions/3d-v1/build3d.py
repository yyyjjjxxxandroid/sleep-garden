...
if bundler:
    subprocess.run([bundler,str(root/'world3d.js'),'--bundle','--format=iife','--minify','--alias:three='+str(root/'vendor/three/build/three.module.js'),'--outfile='+str(root/'world3d.bundle.js')],check=True)
...
assets={p.stem:base64.b64encode(p.read_bytes()).decode() for p in (root/'assets3d').glob('*.glb')}
html=(root/'source.html').read_text()
css=(root/'world.css').read_text()+(root/'world3d.css').read_text()
script='window.GARDEN_ASSETS='+json.dumps(assets)+';\n'+(root/'world3d.bundle.js').read_text()
